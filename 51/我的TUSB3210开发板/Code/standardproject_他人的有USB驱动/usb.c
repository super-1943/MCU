...
extern BYTE code LanguageId[];
extern BYTE code ProductStringDescriptor[];
extern BYTE code SerialNumberStringDescriptor[];
extern BYTE code ManufacturerStringDescriptor[];
...
void usbSetConfiguration(void)
{
	rs232_printstr("SetCFG\r\n");
    usbStallOEP0;
    bConfigurationNumber = tSetupPacket.bValueL;
	tInputEndPointDescriptorBlock[0].bEPCNF |= EPCNF_UBME|EPCNF_USBIE;	
	tInputEndPointDescriptorBlock[0].bEPBCTX = 0x00;
...
{
/*
	rs232_printstr("EP1OUT buffer:");
	rs232_printhex(pbOutBufferAddress[0]);
	rs232_printhex(pbOutBufferAddress[1]);
	rs232_printhex(pbOutBufferAddress[2]);
	rs232_printhex(pbOutBufferAddress[3]);
	rs232_printhex(pbOutBufferAddress[4]);
	rs232_printhex(pbOutBufferAddress[5]);
	rs232_printhex(pbOutBufferAddress[6]);
	rs232_printhex(pbOutBufferAddress[7]);
	rs232_printstr("\r\n");*/
	P0=~pbOutBufferAddress[0];
...
void usbGetConfigurationDescriptor(void)
{
	...
	rs232_printstr("GDC\r\n");
	// Copy the DEVICE DESCRIPTOR from program "ROM" to XRAM
    for(bTemp=0;bTemp<abromConfigurationDescriptorGroup[2];bTemp++)
...
void usbGetStringDescriptor(void)
{
	BYTE bTemp;
	rs232_printstr("GDS");
	rs232_printhex(tSetupPacket.bValueL);
	rs232_printstr("\r\n");
	// Copy the DEVICE DESCRIPTOR from program "ROM" to XRAM
	switch(tSetupPacket.bValueL)
	{
	case 0://Language ID	
	    for(bTemp=0;bTemp<LanguageId[0];bTemp++)
	        abDescriptor[bTemp] = LanguageId[bTemp];
	    usbClearOEP0ByteCount;
	    wBytesRemainingOnIEP0 = LanguageId[0];
		break;
	case 1://Manufacturer
	    for(bTemp=0;bTemp<ManufacturerStringDescriptor[0];bTemp++)
	        abDescriptor[bTemp] = ManufacturerStringDescriptor[bTemp];
	    usbClearOEP0ByteCount;
	    wBytesRemainingOnIEP0 = ManufacturerStringDescriptor[0];
		break;
	case 2://Prod
	    for(bTemp=0;bTemp<ProductStringDescriptor[0];bTemp++)
	        abDescriptor[bTemp] = ProductStringDescriptor[bTemp];
	    usbClearOEP0ByteCount;
	    wBytesRemainingOnIEP0 = ProductStringDescriptor[0];
		break;
	case 3://serial
	    for(bTemp=0;bTemp<SerialNumberStringDescriptor[0];bTemp++)
	        abDescriptor[bTemp] = SerialNumberStringDescriptor[bTemp];
	    usbClearOEP0ByteCount;
	    wBytesRemainingOnIEP0 = SerialNumberStringDescriptor[0];
		break;
	}
    usbSendDataPacketOnEP0((PBYTE)&abDescriptor);
}
...
/*
	rs232_printhex(*(pbEP0_SETUP_ADDRESS+0));
	rs232_printhex(*(pbEP0_SETUP_ADDRESS+1));
	rs232_printhex(*(pbEP0_SETUP_ADDRESS+2));
	rs232_printhex(*(pbEP0_SETUP_ADDRESS+3));
	rs232_printhex(*(pbEP0_SETUP_ADDRESS+4));
	rs232_printhex(*(pbEP0_SETUP_ADDRESS+5));
	rs232_printhex(*(pbEP0_SETUP_ADDRESS+6));
	rs232_printhex(*(pbEP0_SETUP_ADDRESS+7));
	rs232_printstr("\r\n");
*/

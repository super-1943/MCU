...
//求字符串长度
unsigned char strleng(unsigned char *d)
{
	unsigned char i=0;
	while(d[i++]);
	return --i;
...
void bcd2char(unsigned char*d,unsigned char a)
{
	*d++='0'+(a>>4);
	*d++='0'+a&0x0f;
	*d=0;
...
ds12887 port
*****************************/
#define RAM(a) XBYTE[0xB000+a]
unsigned char tb[]={1,0xff,23,0xff,23,0xff,1,20,4,9};
void initram(void)
{
	unsigned char i;
	RAM(0x0b)=0xa6;
